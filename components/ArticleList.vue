...
<template>
  <div class="article-list">
    <article v-for="article in articles" :key="article.path" class="article-row">
      <div class="article-meta"><time :datetime="article.date">{{ date(article.date) }}</time><span v-if="article.tags?.[0]">{{ article.tags[0] }}</span><span>{{ formatReadingTime(article.body, locale) }}</span></div>
