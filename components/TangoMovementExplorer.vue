...
import { centreOfMass, frameAt, sharedCentre, tangoMovements, withEmbrace, withInitiator, type DancerId, type DancerState, type Embrace, type FootState, type MovementId } from '~/utils/tangoSimulation'
...
const selectedInitiator = ref<DancerId>('a')
type Articulation='legato'|'marcato'|'staccato'
type PhaseBinding={countsAsBeat?:boolean;beatUnits:number;articulation:Articulation;connection?:'connected'|'separate'}
type PlaybackInterpretation={id:string;label:string;bindings?:PhaseBinding[];standard?:'legato'|'staccato'}
const selectedInterpretationId=ref('standard:legato')
const editorInterpretations=ref<PlaybackInterpretation[]>([])
const standardInterpretations=computed<PlaybackInterpretation[]>(()=>[
  {id:'standard:legato',label:nl.value?'Legato · rustig en vloeiend':'Legato · calm and fluid',standard:'legato'},
  {id:'standard:staccato',label:nl.value?'Staccato · kort en scherp':'Staccato · short and sharp',standard:'staccato'}
])
...
const loadEditorInterpretations=()=>{try{const saved=JSON.parse(sessionStorage.getItem('tango-editor-movements-v1')||'[]');editorInterpretations.value=Array.isArray(saved)?saved.flatMap((entry:any)=>(entry.data?.musicalInterpretations??[]).map((item:any,index:number)=>{const list=item.phaseBindings??[];return{id:`editor:${entry.id}:${item.id??index}`,label:`${entry.name} · ${item.name||(nl.value?'interpretatie':'interpretation')}`,bindings:list.map((binding:any,bindingIndex:number)=>({countsAsBeat:binding.countsAsBeat??bindingIndex===list.length-1,beatUnits:Number(binding.beatUnits)||1,articulation:binding.articulation??'legato',connection:binding.connection??'connected'}))}})):[];if(![...standardInterpretations.value,...editorInterpretations.value].some(item=>item.id===selectedInterpretationId.value))selectedInterpretationId.value='standard:legato'}catch{editorInterpretations.value=[];selectedInterpretationId.value='standard:legato'}}
const interpretationSegments=computed(()=>{
  const chosen=selectedInterpretation.value,transitions=Math.max(1,movement.value.keyframes.length-1)
  const bindings=chosen.bindings?.length?chosen.bindings:[chosen.standard==='staccato'?{countsAsBeat:true,beatUnits:1,articulation:'staccato' as const,connection:'separate' as const}:{countsAsBeat:true,beatUnits:1,articulation:'legato' as const,connection:'connected' as const}]
  const exact=bindings.length===transitions,groups:{startBinding:number;endBinding:number;beatUnits:number;articulation:Articulation;connection?:'connected'|'separate';startClock:number;endClock:number;startSource:number;endSource:number}[]=[];let start=0
  for(let index=0;index<bindings.length;index++){const binding=bindings[index];if(binding.countsAsBeat||index===bindings.length-1){groups.push({startBinding:start,endBinding:index,beatUnits:Math.max(.25,binding.beatUnits),articulation:binding.articulation,connection:binding.connection,startClock:0,endClock:0,startSource:exact?(start===0?0:movement.value.keyframes[start].at):start/bindings.length,endSource:exact?movement.value.keyframes[index+1].at:(index+1)/bindings.length});start=index+1}}
  const total=groups.reduce((sum,item)=>sum+item.beatUnits,0)||1;let clock=0
  return groups.map(group=>{const startClock=clock;clock+=group.beatUnits/total;return{...group,startClock,endClock:clock}})
})
const articulationCurve=(binding:PhaseBinding,t:number)=>{const u=binding.connection==='separate'?Math.max(0,Math.min(1,(t-.08)/.78)):Math.max(0,Math.min(1,t));if(binding.articulation==='staccato')return Math.min(1,Math.pow(u/.62,.72));if(binding.articulation==='marcato')return 1-Math.pow(1-u,3);return .5-Math.cos(Math.PI*u)/2}
const sourceProgressAt=(clock:number)=>{const p=Math.max(0,Math.min(1,clock)),segments=interpretationSegments.value,segment=segments.find(item=>p<=item.endClock+.0001)??segments.at(-1)!;const local=(p-segment.startClock)/Math.max(.0001,segment.endClock-segment.startClock);return segment.startSource+(segment.endSource-segment.startSource)*articulationCurve(segment,local)}
const motionProgress=computed(()=>sourceProgressAt(progress.value/100))
const clockForSource=(source:number)=>{let low=0,high=1;for(let i=0;i<18;i++){const mid=(low+high)/2;if(sourceProgressAt(mid)<source)low=mid;else high=mid}return Math.round((low+high)/2*100)}
const playbackRate=computed(()=>{const chosen=selectedInterpretation.value;if(chosen.standard==='legato')return .62;if(chosen.standard==='staccato')return 1.28;const segments=interpretationSegments.value,average=segments.length?segments.reduce((sum,item)=>sum+item.beatUnits,0)/segments.length:1;return Math.max(.45,Math.min(1.35,1/average))})
...
const rawFrame = computed(() => frameAt(movement.value, motionProgress.value))
const frame = computed(() => withInitiator(withEmbrace(rawFrame.value, embrace.value, movementId.value), selectedInitiator.value))
...
const initiative = computed(() => {
  const p=motionProgress.value, first=selectedInitiator.value
  return {
...
const actorText = (text:string) => selectedInitiator.value==='a' ? text : text.replace(/\bA\b/g,'§').replace(/\bB\b/g,'A').replace(/§/g,'B')
...
const goToCheckpoint = (index: number) => {
  ...
  if (!target) return
  progress.value = clockForSource(target.at)
  playing.value = false
}
...
watch(playing, active => {
  ...
  if (!active) return
  timer = setInterval(() => {
    progress.value = Math.min(100, progress.value + 0.75*playbackRate.value)
    if (progress.value >= 100) playing.value = false
  }, 35)
})
onMounted(()=>{loadEditorInterpretations();window.addEventListener('tango-editor-movements-changed',loadEditorInterpretations)})
onBeforeUnmount(() => { if (timer) clearInterval(timer);window.removeEventListener('tango-editor-movements-changed',loadEditorInterpretations) })
...
const choicesFor = (id: DancerId) => {
  const initiates = selectedInitiator.value === id
...
const roleFor = (id: DancerId) => selectedInitiator.value === id
...
<template>
  <section class="tango-explorer" data-analytics-model="tango-movement">
    ...
    <fieldset class="initiator-picker">
      <legend>{{ nl ? 'Wie begint deze beweging?' : 'Who starts this movement?' }}</legend>
      <label v-for="id in (['a','b'] as const)" :key="id" :class="{active:selectedInitiator===id}">
        <input v-model="selectedInitiator" type="radio" :value="id">
        {{ nl ? `Danser ${id.toUpperCase()} initieert` : `Dancer ${id.toUpperCase()} initiates` }}
      </label>
      <small>{{ nl ? 'Bij B wordt de volledige bewegingslogica gespiegeld: initiatief, beginbalans en richting wisselen, terwijl de gekozen abrazo behouden blijft.' : 'For B, the complete movement logic is mirrored: initiative, starting balance and direction change while the selected embrace remains intact.' }}</small>
    </fieldset>
    ...
    <div class="stage-card">
      <div class="stage-toolbar">
        ...
        <b class="initiator-key">{{selectedInitiator.toUpperCase()}} · {{ nl ? 'begint het voorstel' : 'starts the proposal' }}</b>
        <span>{{ nl ? 'Fase' : 'Phase' }}: {{ phaseName }}</span>
        <label class="music-choice"><span>{{nl?'Interpretatie':'Interpretation'}}</span><select v-model="selectedInterpretationId"><option v-for="item in playbackInterpretations" :key="item.id" :value="item.id">{{item.label}}</option></select></label>
      ...
      <div class="movement-grammar">
        <span class="system-label">{{ actorText(nl ? movement.systemNl : movement.systemEn) }}</span>
        <ol><li v-for="step in (nl ? movement.sequenceNl : movement.sequenceEn)" :key="step">{{ actorText(step) }}</li></ol>
      </div>
        ...
        <svg class="stage" viewBox="80 70 640 360" role="img" :aria-label="nl ? 'Schematisch bovenaanzicht van twee tangodansers' : 'Schematic top view of two tango dancers'">
          ...
          <g v-for="id in (['a','b'] as const)" :key="id" :class="['dancer', `dancer-${id}`]">
            ...
            <g v-if="initiative[id]>.08" :transform="`translate(${frame[id].torso.x} ${frame[id].torso.y})`" class="initiative" :style="{opacity:initiative[id]}">
              <circle r="54"/><text y="-62">{{ selectedInitiator===id?(nl?'initieert':'initiates'):(nl?'reageert':'responds') }}</text>
            </g>
          </g>
      ...
      <p class="stage-note"><strong>{{ nl ? 'Wat verandert:' : 'What changes:' }}</strong> {{ actorText(nl ? frame.noteNl : frame.noteEn) }}</p>
      <div class="timeline">
      ...
      <section v-if="checkpoint" class="sequence-explorer">
        ...
        <div class="checkpoint-tabs" role="group" :aria-label="nl ? 'Kies stabiele toestand' : 'Choose stable state'">
          <button v-for="(item,index) in checkpoints" :key="item.at" :class="{active:index===checkpointIndex}" @click="goToCheckpoint(index)"><span>{{ index + 1 }}</span>{{ actorText(nl ? item.nameNl : item.nameEn) }}</button>
        </div>
        <div class="checkpoint-detail">
          <div class="checkpoint-state"><small>{{ nl ? 'HUIDIGE TOESTAND' : 'CURRENT STATE' }}</small><strong>{{ actorText(nl ? checkpoint.nameNl : checkpoint.nameEn) }}</strong><p>{{ actorText(nl ? checkpoint.supportNl : checkpoint.supportEn) }}</p></div>
          <div><small>{{ nl ? 'NU BESCHIKBARE TECHNIEKEN' : 'TECHNIQUES AVAILABLE NOW' }}</small><ul><li v-for="item in (nl ? checkpoint.techniquesNl : checkpoint.techniquesEn)" :key="item">{{ actorText(item) }}</li></ul></div>
          <div><small>{{ nl ? 'MOGELIJKE ROUTES' : 'POSSIBLE ROUTES' }}</small><ul><li v-for="item in (nl ? checkpoint.routesNl : checkpoint.routesEn)" :key="item">{{ actorText(item) }}</li></ul></div>
        </div>
    ...
    <div class="views">
      ...
      <section class="agency-card">
        ...
        <div v-if="motionProgress >= .82" class="next-options">
...
.initiator-key{border:1px solid #d18b3b;border-radius:999px;padding:.25rem .55rem;color:#87551e;font-size:.72rem;white-space:nowrap}.initiator-picker{border:0;padding:0;margin:0 0 1rem;display:flex;gap:.5rem;align-items:center;flex-wrap:wrap}.initiator-picker legend{font-weight:750;margin-bottom:.55rem}.initiator-picker label{border:1px solid #d2d8d5;background:#fff;border-radius:999px;padding:.5rem .8rem;cursor:pointer}.initiator-picker label.active{background:#fff4e7;border-color:#9a6729;box-shadow:inset 0 0 0 1px #9a6729}.initiator-picker input{accent-color:#9a6729}.initiator-picker small{width:100%;max-width:760px;color:#697570}
