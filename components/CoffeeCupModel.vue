...
const labels = { mug: 'Herbruikbare mok', paper: 'Papieren beker', plastic: 'Plastic beker' }
...
const unit = computed(() => metric.value === 'climate' ? 'g CO₂-eq' : 'liter water')
const resultValue = (value: number) => metric.value === 'climate' ? whole.format(value) : fmt.format(value)

const sensitivity = computed(() => {
  const candidates = [
    { name: 'Productie-impact mok', effect: (metric.value === 'climate' ? mugProductionClimate.value : mugProductionWater.value) / Math.max(1, horizon.value) },
    { name: 'Elektriciteit voor wassen', effect: washMethod.value === 'dishwasher' ? washClimate.value : handwashEnergy.value * gridIntensity.value / Math.max(1, drinksPerWash.value) },
    { name: 'Consumpties vóór wassen', effect: (metric.value === 'climate' ? washClimate.value : washWater.value) },
    { name: 'Belading vaatwasser', effect: washMethod.value === 'dishwasher' ? (metric.value === 'climate' ? washClimate.value : washWater.value) : 0 },
    { name: 'Impact papieren beker', effect: disposableImpact('paper', metric.value) },
    { name: 'Impact plastic beker', effect: disposableImpact('plastic', metric.value) },
    { name: 'Werkelijke levensduur mok', effect: horizon.value > mugUses.value ? (metric.value === 'climate' ? mugProductionClimate.value : mugProductionWater.value) / mugUses.value : 0 }
  ]
...
async function copyScenario() {
  const summary = `Bekerscenario (${metric.value === 'climate' ? 'klimaat' : 'water'}): ${horizon.value} consumpties; mok ${mugUses.value}× gebruikt; ${drinksPerWash.value} consumpties per wasbeurt; ${washMethod.value === 'dishwasher' ? `vaatwasser, ${dishwasherLoad.value} stuks belading` : 'handwas'}; uitkomst: ${labels[winner.value]}.`
...
<template>
  <section class="cup-model" data-analytics-model="coffee-cups" aria-labelledby="cup-model-title">
    <header class="cup-intro">
      <p class="eyebrow">Interactief bekermodel</p>
      <h2 id="cup-model-title">Wanneer wordt hergebruik de betere keuze?</h2>
      <p>Verander de praktijk, niet alleen het materiaal. De waarden zijn transparante, illustratieve middenwaarden en geen universele LCA-dataset.</p>
    </header>

    <div class="cup-presets" aria-label="Voorbeeldscenario’s">
      <span>Probeer een situatie:</span>
      <button type="button" @click="applyScenario('efficient')">Efficiënt kantoor</button>
      <button type="button" @click="applyScenario('half-full')">Halfvolle vaatwasser</button>
      <button type="button" @click="applyScenario('handwash')">Ruime handwas</button>
      <button type="button" @click="applyScenario('short-life')">Mok kort gebruikt</button>
      <button type="button" @click="applyScenario('disposable')">Goede wegwerpketen</button>
    </div>

    <div class="cup-controls">
      <section aria-labelledby="use-title">
        <h3 id="use-title">Gebruik</h3>
        <label>Vergelijk over <output>{{ whole.format(horizon) }} consumpties</output><input v-model.number="horizon" type="range" min="10" max="2000" step="10"></label>
        <label>Werkelijke levensduur mok <span><input v-model.number="mugUses" type="number" min="1" max="10000" step="25"> keer</span></label>
        <label>Consumpties vóór wassen <span><input v-model.number="drinksPerWash" type="number" min="1" max="10"> keer</span></label>
      </section>
      <section aria-labelledby="wash-title">
        <h3 id="wash-title">Afwas</h3>
        <div class="cup-toggle" role="group" aria-label="Wasmethode"><button type="button" :class="{ active: washMethod === 'dishwasher' }" @click="washMethod = 'dishwasher'">Vaatwasser</button><button type="button" :class="{ active: washMethod === 'hand' }" @click="washMethod = 'hand'">Handwas</button></div>
        <template v-if="washMethod === 'dishwasher'">
          <label>Energie per cyclus <span><input v-model.number="dishwasherEnergy" type="number" min="0" max="5" step="0.05"> kWh</span></label>
          <label>Water per cyclus <span><input v-model.number="dishwasherWater" type="number" min="0" max="50" step="0.5"> liter</span></label>
          <label>Belading <span><input v-model.number="dishwasherLoad" type="number" min="1" max="200"> stuks</span></label>
        </template>
        <template v-else>
          <label>Energie per mok <span><input v-model.number="handwashEnergy" type="number" min="0" max="2" step="0.01"> kWh</span></label>
          <label>Water per mok <span><input v-model.number="handwashWater" type="number" min="0" max="30" step="0.5"> liter</span></label>
        </template>
      </section>
      <section aria-labelledby="disposable-title">
        <h3 id="disposable-title">Wegwerppraktijk</h3>
        <label>Gebruik papieren beker <span><input v-model.number="paperUses" type="number" min="1" max="5"> keer</span></label>
        <label>Inzameling papier <span><input v-model.number="paperCollection" type="number" min="0" max="100" step="5"> %</span></label>
        <label>Gebruik plastic beker <span><input v-model.number="plasticUses" type="number" min="1" max="5"> keer</span></label>
        <label>Inzameling plastic <span><input v-model.number="plasticCollection" type="number" min="0" max="100" step="5"> %</span></label>
      </section>
    </div>

    <details class="cup-advanced">
      <summary>Geavanceerde rekenaannames aanpassen</summary>
      <div>
        <label>Productie mok <span><input v-model.number="mugProductionClimate" type="number" min="1" step="10"> g CO₂-eq</span></label>
        <label>Productiewater mok <span><input v-model.number="mugProductionWater" type="number" min="0" step="0.5"> liter</span></label>
        <label>Papieren beker <span><input v-model.number="paperClimate" type="number" min="0" step="1"> g CO₂-eq</span></label>
        <label>Water papier <span><input v-model.number="paperWater" type="number" min="0" step="0.05"> liter</span></label>
        <label>Plastic beker <span><input v-model.number="plasticClimate" type="number" min="0" step="1"> g CO₂-eq</span></label>
        <label>Water plastic <span><input v-model.number="plasticWater" type="number" min="0" step="0.01"> liter</span></label>
        <label>Elektriciteitsfactor <span><input v-model.number="gridIntensity" type="number" min="0" max="1000" step="10"> g CO₂-eq/kWh</span></label>
        <label>Recyclingvoordeel <span><input v-model.number="recyclingBenefit" type="number" min="0" max="80" step="5"> %</span></label>
        <label>Onzekerheidsmarge <span><input v-model.number="uncertainty" type="number" min="0" max="75" step="5"> ±%</span></label>
      </div>
    </details>

    <div class="cup-results" aria-live="polite">
      <div><span>Laagste impact bij {{ whole.format(horizon) }} consumpties</span><strong>{{ labels[winner] }}</strong></div>
      <div><span>Omslagpunt t.o.v. papier</span><strong>{{ paperBreakEven ? `${whole.format(paperBreakEven)}×` : 'niet ≤ 5.000×' }}</strong></div>
      <div><span>Omslagpunt t.o.v. plastic</span><strong>{{ plasticBreakEven ? `${whole.format(plasticBreakEven)}×` : 'niet ≤ 5.000×' }}</strong></div>
    </div>

    <div class="cup-chart-heading">
      <div><h3>Cumulatieve impact</h3><p>De transparante zones tonen de ingestelde onzekerheidsmarge.</p></div>
      <div class="cup-toggle" role="group" aria-label="Milieucategorie"><button type="button" :class="{ active: metric === 'climate' }" @click="metric = 'climate'">Klimaat</button><button type="button" :class="{ active: metric === 'water' }" @click="metric = 'water'">Water</button></div>
    </div>
    <div class="cup-chart-scroll" tabindex="0">
      <svg class="cup-chart" :viewBox="`0 0 ${chart.width} ${chart.height}`" role="img" aria-labelledby="cup-chart-title cup-chart-desc">
        <title id="cup-chart-title">Cumulatieve milieu-impact van drie bekersystemen</title>
        <desc id="cup-chart-desc">Lijnen vergelijken een herbruikbare mok, papieren beker en plastic beker over het gekozen aantal consumpties.</desc>
        ...
        <text :x="chart.margin.left + chart.innerWidth / 2" :y="chart.height - 2" text-anchor="middle" class="axis-title">Aantal consumpties</text>
        <text :transform="`translate(16 ${chart.margin.top + chart.innerHeight / 2}) rotate(-90)`" text-anchor="middle" class="axis-title">{{ unit }}</text>
      </svg>
    </div>
    <div class="cup-legend"><span class="mug"><i/>Herbruikbare mok</span><span class="paper"><i/>Papieren beker</span><span class="plastic"><i/>Plastic beker</span></div>

    <div class="cup-per-use">
      <h3>Impact per consumptie bij deze gebruiksduur</h3>
    ...
    <div class="cup-sensitivity"><h3>Meest bepalende aannames in dit scenario</h3><ol><li v-for="item in sensitivity" :key="item.name">{{ item.name }}</li></ol><p>Dit is een lokale gevoeligheidsindicatie, geen statistische onzekerheidsanalyse.</p></div>
    <footer class="cup-notes">
      <p><strong>Rekenwijze.</strong> Productie wordt verdeeld over de feitelijke levensduur. Iedere washandeling of nieuwe wegwerpbeker telt opnieuw mee. Het recyclingvoordeel geldt alleen voor het daadwerkelijk ingezamelde aandeel.</p>
      <p><strong>Lees de uitkomst voorwaardelijk.</strong> De middenwaarden zijn illustratief. Vul voor besluitvorming gegevens in die bij de gekozen beker, vaatwasser, energiemix en afvalketen horen. Klimaat en water blijven afzonderlijke indicatoren.</p>
      <button type="button" @click="copyScenario">{{ copied ? 'Scenario gekopieerd' : 'Kopieer samenvatting van scenario' }}</button>
    </footer>
  </section>
</template>
