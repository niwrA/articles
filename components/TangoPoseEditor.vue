...
type Pose={id:string;at:number;phase:PhaseName;note:string;a:Dancer;b:Dancer}
...
const initial=():Pose[]=>[
 {id:crypto.randomUUID(),at:0,phase:'available',note:'',a:dancer(330,250,0,foot(310,232,0,0),foot(310,268,0,1)),b:dancer(470,250,180,foot(490,268,180,0),foot(490,232,180,1))},
 {id:crypto.randomUUID(),at:1,phase:'arrival',note:'',a:dancer(330,250,0,foot(310,232,0,0),foot(310,268,0,1)),b:dancer(470,250,180,foot(490,268,180,0),foot(490,232,180,1))}
]
...
const duplicate=()=>{const source=clone(current.value),nextAt=selected.value<poses.value.length-1?(source.at+poses.value[selected.value+1].at)/2:Math.min(1,source.at+.1);source.id=crypto.randomUUID();source.at=nextAt;poses.value.splice(selected.value+1,0,source);normalise();choose(selected.value+1)}
const remove=()=>{if(poses.value.length<=2)return;poses.value.splice(selected.value,1);selected.value=Math.min(selected.value,poses.value.length-1);normalise()}
const normalise=()=>{const activeId=current.value?.id;poses.value.sort((a,b)=>a.at-b.at);poses.value[0].at=0;poses.value.at(-1)!.at=1;selected.value=Math.max(0,poses.value.findIndex(x=>x.id===activeId))}
...
const payload=()=>({format:'tango-pose-sequence',version:1,coordinateSystem:{view:'top',origin:'stage',angleDegrees:'clockwise',units:'model'},poses:poses.value.map(({id,...p})=>p)})
...
const importJson=()=>{try{const parsed=JSON.parse(jsonInput.value),incoming=Array.isArray(parsed)?parsed:parsed.poses;if(!Array.isArray(incoming)||incoming.length<2)throw new Error('poses');poses.value=incoming.map((p:Pose)=>({...p,id:crypto.randomUUID()}));selected.value=0;normalise();message.value=nl.value?'Sequentie geladen':'Sequence loaded'}catch{message.value=nl.value?'Ongeldige JSON':'Invalid JSON'}}
</script>
 ...
 <section class="pose-editor">
  ...
  <div class="editor-grid">
   ...
   <aside class="controls">
    ...
    <label>{{nl?'Moment':'Time'}} <input v-model.number="current.at" type="range" min="0" max="1" step=".01" :disabled="selected===0||selected===poses.length-1" @change="normalise"><output>{{Math.round(current.at*100)}}%</output></label>
    ...
    <details v-for="id in (['a','b'] as DancerId[])" :key="id" :open="id==='b'"><summary>{{nl?'Danser':'Dancer'}} {{id.toUpperCase()}}</summary>
     <div class="angle-grid"><label>{{nl?'bovenlijf':'upper body'}} <input v-model.number="current[id].angle" type="number" step="5">°</label><label>{{nl?'bekken':'pelvis'}} <input v-model.number="current[id].pelvisAngle" type="number" step="5">°</label></div>
     <div v-for="side in (['left','right'] as Side[])" :key="side" class="foot-controls"><strong>{{side==='left'?(nl?'Linkervoet':'Left foot'):(nl?'Rechtervoet':'Right foot')}}</strong><label>x <input v-model.number="current[id][side].x" type="number"></label><label>y <input v-model.number="current[id][side].y" type="number"></label><label>{{nl?'hoek':'angle'}} <input v-model.number="current[id][side].angle" type="number" step="5"></label><label>{{nl?'gewicht':'load'}} <input v-model.number="current[id][side].load" type="range" min="0" max="1" step=".05"><output>{{Math.round(current[id][side].load*100)}}%</output></label></div>
    </details>
