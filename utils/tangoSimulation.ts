...
export type BiomechanicalCheck={id:'reach'|'torsion'|'load'|'clearance'|'balance'|'support'|'orientation';ok:boolean;labelNl:string;labelEn:string}
...
export function assessMovementAt(movement:Movement,progress:number):BiomechanicalCheck[]{const frame=rawFrameAt(movement,progress),epsilon=.0025,before=rawFrameAt(movement,Math.max(0,progress-epsilon)),after=rawFrameAt(movement,Math.min(1,progress+epsilon)),supportStable=(['a','b'] as const).every(id=>(['left','right'] as const).every(side=>before[id][side].load<=.85||after[id][side].load<=.85||distance(before[id][side],after[id][side])<=1.8)),towards=(from:DancerState,to:DancerState)=>direction(from.torso,to.torso),endAligned=progress<.995||(Math.abs(signedAngleDelta(frame.a.angle,towards(frame.a,frame.b)))<=20&&Math.abs(signedAngleDelta(frame.b.angle,towards(frame.b,frame.a)))<=20);return[...assessBiomechanics(frame),{id:'support',ok:supportStable,labelNl:'Belaste standvoeten schuiven niet',labelEn:'Loaded support feet do not slide'},{id:'orientation',ok:endAligned,labelNl:'Eindhouding is wederzijds georiënteerd',labelEn:'Final stance is mutually oriented'}]}
